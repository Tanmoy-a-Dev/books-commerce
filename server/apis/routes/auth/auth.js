...
router.post('/verify-email/:userId', async (req, res, next) => {
  try {
    ...
    if (!user || user.activated) {
      res
        .status(500)
        .json({ msg: "You don't have the privilige to do this action !" });
    }
    ...
    if (
      ...
      res
        .status(401)
        .json({ msg: 'Wait for 2 minutes before sending another email' });
    }
  } catch (error) {
    console.error(error);
    next(error);
...
const { ServiceUnavailableError } = require('../../../helpers/customErrors');
router.get('/check-token/:emailToken', async (req, res) => {
  try {
    ...
    } else throw new ServiceUnavailableError('Token is invalid');
  } catch (error) {
